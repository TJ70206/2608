...
3. 无标签退化一致性约束：在目标域训练中加入 RUL 单调性、代理 HI 单调性或同阶段平滑约束，抑制负迁移。
...
4. 目标域无标签伪时间阶段对齐：第二迁移已修正为 `target_stage_source: time_progress`，避免 RUL 标签泄漏。
...
```text
PG-STDA 在 P-SA-MCD-TCN 表征骨干上，引入航天机理代理特征、伪时间退化阶段划分和无标签退化一致性约束，在不使用目标域 RUL 标签的条件下完成公开退化源域到航天仿真目标域的阶段感知迁移。
```
...
这比直接加入 target supervision 更符合严格迁移，也更容易解释为创新。
...
3. 在严格无监督迁移中加入 `target_monotonic_loss`，先网格试 `0.001, 0.003, 0.01`。
4. 两个迁移均跑同一方法，验证是否能提升第二迁移 P-SA-MCD。
...
```text
本文提出一种面向航天器关键组件寿命预测的机理约束伪阶段跨域迁移框架 PG-STDA。该框架不直接依赖目标域寿命标签，而是利用公开退化源域学习通用退化表征，并在航天仿真目标域中通过可观测遥测构造机理代理健康特征，结合伪时间阶段划分和局部 MMD 完成同退化阶段对齐。同时，引入无标签退化一致性约束，提升目标域预测轨迹的单调性、稳定性和工程可解释性。
```
